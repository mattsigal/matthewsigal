...
```{r}
t.test(dep1 ~ age, data=dat, var.equal=TRUE)  # Student
t.test(dep1 ~ age, data=dat, var.equal=FALSE) # Welch
```
...
```{r}
t.test(dat$perf3, dat$perf1, paired=TRUE)
```
...
```{r}
mod.aov <- aov(perf3 ~ group, data=dat)
summary(mod.aov)
TukeyHSD(mod.aov)  # Non-significant ombnibus, but what the heck
```

6. Test a factorial ANOVA for group by age category on perfection scores at time 3.

```{r}
mod.aov2 <- aov(perf3 ~ group * age, data=dat)
summary(mod.aov2)
interaction.plot(dat$group, dat$age, dat$perf3)
```
...
```{r}
# Linear Regresion
ggplot(dat, aes(y = perf1, x = dep1)) + geom_point() + geom_smooth(method = "lm")
reg.mod <- lm(perf1 ~ dep1, data = dat)
summary( reg.mod )

# Multiple regression
reg.mod2 <- lm(perf3 ~ dep1 + perf1, data = dat)
summary(reg.mod2)

reg.mod3 <- lm(perf3 ~ dep1 * perf1, data = dat)
summary(reg.mod3)
```